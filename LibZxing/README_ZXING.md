# Библиотека Zxing 
Данная библиотека преобразует исходное изображение в бинарное изображение, после чего (в нашем случае, для проверки работы на основных видах штрихкодов) полученное бинарное изображение передается в метод ```decodeInternal```, в котором перебираются все декодеры (для каждого типа штрихкода), с вызовом метода ```decode``` для нахождения особых признаков соответствующего штрихкода на бинарном изображении.
